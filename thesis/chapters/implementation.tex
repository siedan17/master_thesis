...
F\"ur die Implementierung der unterschiedlichen Machine Learning Modelle wurde vor allem die Bibliothek \textit{Scikit-Learn} \cite{sklearn} verwendet, welche eine einfache API f\"ur 
...
F\"ur die Implementierung der k\"unstlichen neuronalen Netzwerke wurde \textit{Tensorflow} \cite{tensorflow} und die passende API von \textit{Keras} \cite{keras} verwendet. \\
...
One-Hot-Encoding bedeutet, dass ein nominales Merkmal mit $m$ Klassen in $m-1$ neue Merkmale umcodiert wird. Danach ist jenes Merkmale, welches 
...
es kein neues Merkmal gibt, so entspricht es der Kodierung, dass alle neuen Merkmale 0 sind \cite{handson}. \\
...
Diese Unterteilung wurde so durch gef\"uhrt, dass die jeweiligen Datens\"atze pseudo-randomisiert zusammengestellt wurden,
und auch der Anteil an pr\"ufungsaktiven Studierenden \"uberall gleich ist. Diesen Vorgang nennt man \textit{Stratifizierung} \cite{handson}. Zus\"atzlich wurde bei der Auswertung des Trainings der 
Algorithmen \textit{Crossvalidation} verwendet. \\

Crossvalidation bedeuted, dass der Trainingsdatensatz in $k$ gleichm\"achtige Mengen zerteilt wird, die zuf\"allig gebildet werden. 
...
und sieht auch deren Verteilung \cite{handson}. \\
...
Merkmale aus bereits vorhanden Merkmalen berechnet. Hier wurden auch entschieden, ob Merkmale verwendet oder verworfen werden. 
